...
generar predicciones que contrasten los huracanes con nombres masculinos y femeninos. 
...
#Gráfico Dispersión altura vs predictor 1: peso.centrado
g_2c_feminidad.std_prediccion.muertes_fpd_negbin<-tabla_2c %>% 
  ggplot(aes(x=feminidad.std,y=prediccion.muertes_fpd_negbin,color=factor(genero))) +
  theme(legend.position="bottom")+
  ylim(0,260)+
  geom_jitter(data=tabla_2c, # %>% #agregamos los datos de la muestra
               aes(feminidad.std,prediccion.muertes_fpd_negbin),
               height = 0, width = 0.01) +
  labs(title = "Pred muertes vs Fem.std")+
  theme_classic()

g_2c_feminidad_muertes_genero+g_2c_feminidad.std_prediccion.muertes_fpd_negbin
```

AQUI NO SÉ SI HAY QUE HACER TODAS LAS GRÁFICAS O REVISAR EL ERROR ABSOLUTO PROMERIO CON EL GÉNERO PARA CONCLUIR O SÓLO EL GRÁFICO.
...
HAY QUE HACER LO MISMO CON EL RESTO DE LOS COEFS
...
g_2d_logdamage_norm_muertes_pred<-tabla_2c_dm %>% 
  ggplot(aes(x=log(damage_norm),y=prediccion.muertes)) +
  ylim(0,260)+
  geom_point()+
  geom_smooth(method = "lm")+
