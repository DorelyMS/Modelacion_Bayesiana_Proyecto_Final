...
```{r}
mean(inv_logit_scaled(post$b_Intercept)) %>%
  round(digits = 2)
```
...
```{r}
mean(inv_logit_scaled(post$b_Intercept+post$b_pirataGrande))%>%
  round(digits = 2)
```

Large-bodied pirates win almost all of the time (87%)! We will repeat this for all slope parameters:

```{r}
mean(inv_logit_scaled(post$b_Intercept+post$b_victimaGrande))%>%
  round(digits = 2)
```

```{r}
mean(inv_logit_scaled(post$b_Intercept+post$b_pirataAdulto))%>%
  round(digits = 2)
```
